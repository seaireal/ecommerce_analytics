...
library(e1071) # svm
library(ROSE) # sample unbalanced class
...
```
# Over sampling (caret package)
```{r}
set.seed(1)
up_train <- upSample(x = df[, -ncol(df)],
                     y = df$Revenue) 
names(up_train)[18]="Revenue"
table(up_train$Revenue) 

df_0 = up_train[up_train$Revenue == 0, ]
df_1 = up_train[up_train$Revenue == 1, ]

split_size = as.integer(nrow(df_1)/5)
train_index = sample(1:nrow(df_1), split_size)

df_0_train = df_0[-train_index, ]
df_0_test = df_0[train_index, ]
df_1_train = df_1[-train_index, ]
df_1_test = df_1[train_index, ]
...
```{r}
balanced.data <- ROSE(Revenue ~., data=df)$data
table(balanced.data$Revenue)

df_0 = balanced.data[balanced.data$Revenue == 0, ]
df_1 = balanced.data[balanced.data$Revenue == 1, ]
...
# DNN
## With deepnet: single layer
```{r}
# data processing for deepnet package
...
x_train = as.matrix(df_train_c[,-18])
x_test = as.matrix(df_test_c[,-18])
y_train = as.matrix(df_train_c[,18])
y_test = as.matrix(df_test_c[,18])
...
```{r}
nn <- nn.train(x_train, y_train, hidden = c(1, 5, 10), activationfun="sigm", learningrate=0.8, momentum=0.5, learningrate_scale=1, output="sigm", numepochs=3, batchsize=100, hidden_dropout=0, visible_dropout=0)
...
print("sensitivity")
sum(yhat==1 & y_test==1)/sum(y_test==1)

print("specificity")
sum(yhat==0 & y_test==0)/sum(y_test==0)

roc(y_test, yhat, levels = c(0, 1), direction = "<") %>% auc()
```
...
num.tr=100
num.te=10000
X=matrix(rnorm(num.tr*2),ncol=2)
A=cbind(c(3,3),c(3,-3))
X.derived=X%*%A
Y.mean=sigmoid(X.derived[,1])+sigmoid(X.derived[,2])
Y=Y.mean+sigma.1*rnorm(num.tr)
model.1.tr=data.frame(Y=Y,X)
X=matrix(rnorm(num.te*2),ncol=2)
A=cbind(c(3,3),c(3,-3))
X.derived=X%*%A
Y.mean=sigmoid(X.derived[,1])+sigmoid(X.derived[,2])
Y=Y.mean+sigma.1*rnorm(num.te)
model.1.te=data.frame(Y=Y,X)
X=matrix(rnorm(num.tr*10),ncol=10)
X.derived=dnorm(X)*sqrt(2*pi)
Y.mean=apply(X.derived,MARGIN=1,FUN=prod)
Y=Y.mean+sigma.2*rnorm(num.tr)
model.2.tr=data.frame(Y=Y,X)
X=matrix(rnorm(num.te*10),ncol=10)
X.derived=dnorm(X)
Y.mean=apply(X.derived,MARGIN=1,FUN=prod)
Y=Y.mean+sigma.2*rnorm(num.te)
model.2.te=data.frame(Y=Y,X)
```

Example: Number of hidden units
```{r}
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = 3, decay = 5e-4,
linout = T, skip = F, maxit = 1000, Hess = T)
sum((predict(model.1.nn,model.1.te)-model.1.te$Y)^2)/num.te/(sigma.1^2)
error.1=array(0,dim=c(10,10))
for (i in 1:10){
for (j in 1:10){
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = i, decay = 1e-2,
linout = T, skip = F, maxit = 1000, Hess = T, trace=F)
error.1[i,j]=sum((predict(model.1.nn,model.1.te)-model.1.te$Y)^2)/num.te/(sigma.1^2)
}
}
error.1.lin=sum((predict(lm(Y ~ X1 + X2, model.1.tr),model.1.te)-model.1.te$Y)^2)
error.1.lin=error.1.lin/num.te/(sigma.1^2)
plot(as.factor(c(rep(1:10,10),0)),c(as.vector(error.1),error.1.lin),
xlab="Number of Hidden Units",ylab="Relative Test Error",main="Sum of Sigmoids")
error.2=array(0,dim=c(10,10))
for (i in 1:10){
for (j in 1:10){
model.2.nn=nnet(Y ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9 + X10, model.2.tr,
size = i, decay = .01, linout = T, skip = F, maxit = 1000, Hess = T, trace=F)
error.2[i,j]=sum((predict(model.2.nn,model.2.te)-model.2.te$Y)^2)/num.te/(sigma.2^2)
}
}
error.2.lin=sum((predict(lm(Y ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10,model.2.tr),
model.2.te)-model.2.te$Y)^2)
error.2.lin=error.2.lin/num.te/(sigma.2^2)
plot(as.factor(c(rep(1:10,10),0)),c(as.vector(error.2),error.2.lin),
xlab="Number of Hidden Units",ylab="Relative Test Error",main="Radial")
```
...
for (i in 1:10){
for (j in 1:10){
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = i, decay = 0,
linout = T, skip = F, maxit = 5000, Hess = T, trace=F)
error.1[i,j]=sum((predict(model.1.nn,model.1.te)-model.1.te$Y)^2)/num.te/(sigma.1^2)
}
}
error.1.lin=sum((predict(lm(Y ~ X1 + X2, model.1.tr),model.1.te)-model.1.te$Y)^2)
error.1.lin=error.1.lin/num.te/(sigma.1^2)
plot(as.factor(c(rep(1:10,10),0)),c(as.vector(error.1),error.1.lin),
xlab="Number of Hidden Units",ylab="Relative Test Error",
main="No Weight Decay",ylim=c(1,5))
error.1=array(0,dim=c(10,10))
for (i in 1:10){
for (j in 1:10){
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = i, decay = .1,
linout = T, skip = F, maxit = 1000, Hess = T, trace=F)
error.1[i,j]=sum((predict(model.1.nn,model.1.te)-model.1.te$Y)^2)/num.te/(sigma.1^2)
}
}
...
plot(as.factor(c(rep(1:10,10),0)),c(as.vector(error.1),error.1.lin),
xlab="Number of Hidden Units",ylab="Relative Test Error",
main="Weight Decay = 0.1",ylim=c(1,5))
```

```{r}
lambda=0:15/100
error.1=array(0,dim=c(16,10))
for (i in 1:16){
for (j in 1:10){
if (i == 1){
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = 10, decay = lambda[i],
linout = T, skip = F, maxit = 5000, Hess = T, trace=F)}
if (i != 1){
model.1.nn=nnet(Y ~ X1 + X2, model.1.tr, size = 10, decay = lambda[i],
linout = T, skip = F, maxit = 2000, Hess = T, trace=F)}
error.1[i,j]=sum((predict(model.1.nn,model.1.te)-model.1.te$Y)^2)/num.te/(sigma.1^2)
}
}
plot(as.factor(rep(1:16/100,10)),as.vector(error.1),xlab="Weight Decay Parameter",
ylab="Relative Test Error",main="Sum of Sigmoids, 10 Hidden Units Model",
labels=lambda,ylim=c(1,3))
```
