...
idx = sample(1:nrow(df_0), nrow(df_1))
df_0_sub = df_0[idx, ]
df_0_re = df_0[-idx, ]
...
## Regular sampling
```{r}
itm = rbind(df_0_train, df_0_re)
itm = rbind(itm, df_1_train)
table(itm$Revenue)
df_train = itm
table(df_train$Revenue)
table(df_test$Revenue)
...
# Oversampling (caret package)
```{r}
set.seed(1)
df_train <- upSample(x = itm[, -ncol(itm)],
                     y = itm$Revenue) 
names(df_train)[18]="Revenue"
table(df_train$Revenue) 
```

## ROSE: synthetic data generation
```{r}
df_train <- ROSE(Revenue ~., data=itm)$data
table(df_train$Revenue)

df_train=df_train[!is.na(df_train$Revenue),]
df_test=df_test[!is.na(df_test$Revenue),]
table(df_train$Revenue)
```
...
y_train = as.matrix(df_train_c$Revenue)
y_test = as.matrix(df_test_c$Revenue)
...
for (i in 1:16){
  for (j in 1:10){
    if (i == 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 10, decay = lambda[i], linout = T, skip = F, maxit = 5000, Hess = T, trace=F)}
    if (i != 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 10, decay = lambda[i], linout = T, skip = F, maxit = 2000, Hess = T, trace=F)}
    error.1[i,j]=sum((predict(model.1.nn,df_test_c)-df_test_c$Revenue)^2)/nrow(df_test_c)
  }
}
plot(as.factor(rep(1:16/100,10)),as.vector(error.1),xlab="Weight Decay Parameter",ylab="Relative Test Error",main="Sum of Sigmoids, 10 Hidden Units Model", labels=lambda)

lambda=0:15/100
error.1=array(0,dim=c(16,15))
for (i in 1:16){
  for (j in 1:15){
    if (i == 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 15, decay = lambda[i], linout = T, skip = F, maxit = 5000, Hess = T, trace=F)}
    if (i != 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 15, decay = lambda[i], linout = T, skip = F, maxit = 2000, Hess = T, trace=F)}
    error.1[i,j]=sum((predict(model.1.nn,df_test_c)-df_test_c$Revenue)^2)/nrow(df_test_c)
  }
}
plot(as.factor(rep(1:16/100,15)),as.vector(error.1),xlab="Weight Decay Parameter",ylab="Relative Test Error",main="Sum of Sigmoids, 10 Hidden Units Model", labels=lambda)

lambda=0:15/100
error.1=array(0,dim=c(16,20))
for (i in 1:16){
  for (j in 1:20){
    if (i == 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 20, decay = lambda[i], linout = T, skip = F, maxit = 5000, Hess = T, trace=F)}
    if (i != 1){
      model.1.nn=nnet(Revenue ~ ., df_train_c, size = 20, decay = lambda[i], linout = T, skip = F, maxit = 2000, Hess = T, trace=F)}
    error.1[i,j]=sum((predict(model.1.nn,df_test_c)-df_test_c$Revenue)^2)/nrow(df_test_c)
  }
}
plot(as.factor(rep(1:16/100,20)),as.vector(error.1),xlab="Weight Decay Parameter",ylab="Relative Test Error",main="Sum of Sigmoids, 10 Hidden Units Model", labels=lambda)

```

```{r}
sensitivity=array(0,dim=c(1,15))
specificity=array(0,dim=c(1,15))
auc=array(0,dim=c(1,15))
error.1=array(0,dim=c(1,15))
for (j in 1:15){
  model.1.nn=nnet(Revenue ~ ., df_train_c, size = 15, decay = 0.01, linout = T, skip = F, maxit = 5000, Hess = T, trace=F)
  model.1.pred=predict(model.1.nn,df_test_c)
      
  yhat = ifelse(model.1.pred > 0.5, 1, 0)
  sensitivity[,j]=sum(yhat==1 & df_test_c$Revenue==1)/sum(df_test_c$Revenue==1)
  specificity[,j]=sum(yhat==0 & df_test_c$Revenue==0)/sum(df_test_c$Revenue==0)
  auc[,j]=roc(df_test_c$Revenue, yhat, levels = c(0, 1), direction = "<") %>% auc()

  error.1[,j]=mean(df_test_c$Revenue != yhat)

}
plot(as.factor(rep(0.15,15)),as.vector(error.1),xlab="Weight Decay Parameter",ylab="Relative Test Error",main="Sum of Sigmoids, 15 Hidden Units Model", labels=lambda)
```
...
```{r}
model.1.nn=nnet(Revenue ~ ., df_train_c, size = 20, decay = 0.15, linout = T, skip = F, maxit = 5000, Hess = T, trace=F)
model.1.pred=predict(model.1.nn,df_test_c)
      
yhat = ifelse(model.1.pred > 0.5, 1, 0)
sensitivity=sum(yhat==1 & df_test_c$Revenue==1)/sum(df_test_c$Revenue==1)
specificity=sum(yhat==0 & df_test_c$Revenue==0)/sum(df_test_c$Revenue==0)
auc=roc(df_test_c$Revenue, yhat, levels = c(0, 1), direction = "<") %>% auc()

error.1=mean(df_test_c$Revenue != yhat)
...
train_index = sample(1:nrow(df_0), split_size)
...
```{r}
set.seed(1)
df_0 = itm[itm$Revenue == 0, ]
df_1 = itm[itm$Revenue == 1, ]
idx = sample(1:nrow(df_0), nrow(df_1))
df_0_sub = df_0[idx, ]
df_0_re = df_0[-idx, ]

train_index = sample(1:nrow(df_1), nrow(df_1))
df_0_train = df_0_sub[train_index, ]

df_train = rbind(df_0_train, df_1)
