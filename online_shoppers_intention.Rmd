...
predicted = predict(tune_svm1$best.model, df_test, probability = TRUE)
test_revenue = df_test$Revenue

print("test error")
mean(predicted != test_revenue)

print("sensitivity")
sum(predicted==1 & test_revenue==1)/sum(test_revenue==1)

print("specificity")
sum(predicted==0 & test_revenue==0)/sum(test_revenue==0)

roc(test_revenue, attributes(predicted)$probabilities[,2], levels = c(0, 1), direction = "<") %>% auc()
```
