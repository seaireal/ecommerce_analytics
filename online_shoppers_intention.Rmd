...
## With Keras
```{r}
# data processing for deepnet and Keras package
x_train = as.matrix(df_train[,1:17])
x_test = as.matrix(df_test[,1:17])
y_train = as.matrix(df_train[,18])
y_test = as.matrix(df_test[,18])
...
# 
# print("sensitivity")
# sum(Y_test_hat==1 & y_test==1)/sum(y_test==1)
# 
# print("specificity")
# sum(Y_test_hat==0 & y_test==0)/sum(y_test==0)
# 
# roc(y_test, attributes(Y_test_hat)$probabilities[,2], levels = c(0, 1), direction = "<") %>% auc()
```
...
```{r}
nn <- nn.train(x_train, y_train, hidden = c(5))
...
# yhat <- keras_predict_classes(mod, X_test)
# table(y_test, yhat)
# 
print("test error")
mean(y_test != yhat)
